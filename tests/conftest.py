...
* the run metrics history (``frontend/metrics.json``);
* the published report documents (``frontend/reports/``);
...
All of them are redirected into ``tmp_path`` for every test and the
process-wide ledger and LLM call record are reset, which keeps the suite
side-effect free and each test independent. Tests that care about a particular
file set their own explicit path.
"""
...
from ai_assistant import memory, metrics, reports
...
@pytest.fixture(autouse=True)
def _metrics_file_in_tmp(monkeypatch, tmp_path):
    monkeypatch.setenv(metrics.METRICS_FILE_ENV, str(tmp_path / "metrics.json"))


@pytest.fixture(autouse=True)
def _reset_call_stats():
    """No test may see the token counters another test's fake call left behind."""
...
@pytest.fixture(autouse=True)
def _reports_dir_in_tmp(monkeypatch, tmp_path):
    monkeypatch.setenv(reports.REPORTS_DIR_ENV, str(tmp_path / "reports"))

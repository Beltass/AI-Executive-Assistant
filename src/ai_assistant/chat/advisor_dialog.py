...
class AdvisorDialogManager:
    ...
    def get_dialog(self, advisor_key: str, user_id: str) -> Optional[AdvisorDialog]:
        ...
        try:
            ...
            logger.warning(
                f"Dialog yükleme başarısız ({advisor_key}, {user_id}): {exc}"
            )
        return None
    ...
    def get_or_create_dialog(self, advisor_key: str, user_id: str) -> AdvisorDialog:
...
class AdvisorDialogFlow:
    ...
    def handle_advisor_selection(
        self, user_id: str, selection: str
    ...
    def handle_advisor_input(
        self, advisor_key: str, user_id: str, user_input: str
    ) -> Dict[str, Any]:
        ...
        dialog.conversation_history.append(
    ...
    def _handle_data_analyst_input(
        self, dialog: AdvisorDialog, user_input: str
    ) -> Dict[str, Any]:
        ...
        if any(
            ...
            for keyword in ["csv", "json", "excel", "yükle", "paste", "yapıştır"]
        ):
        ...
        if any(
            keyword in user_input_lower
            for keyword in ["trend", "korelasyon", "özet", "summary"]
        ):
        ...
        if any(
            keyword in user_input_lower
            for keyword in ["excel", "dashboard", "rapor", "csv", "json"]
        ):
    ...
    def _data_analyst_process_analysis(
        self, dialog: AdvisorDialog, user_input: str
    ) -> Dict[str, Any]:
        """Veri analizi işle."""
        analysis_type = (
            ...
            else "Korelasyon" if "korelasyon" in user_input.lower() else "Özet"
        )
    ...
    def _data_analyst_format_output(
        self, dialog: AdvisorDialog, user_input: str
    ) -> Dict[str, Any]:
        ...
        output_format = (
            ...
            if "excel" in user_input.lower()
                ...
                else "Rapor" if "rapor" in user_input.lower() else "CSV"
            )
        )
    ...
    def _generate_sample_analysis_report(
        self, dialog: AdvisorDialog, format_type: str
    ) -> str:
        """Örnek analiz raporu oluştur."""
        analysis_type = dialog.temporary_data.get("analysis_type", "Özet")
        return (
            f"📊 *{analysis_type} Analizi Raporu*\n\n"
    ...
    def _handle_social_media_coach_input(
        self, dialog: AdvisorDialog, user_input: str
    ) -> Dict[str, Any]:
        ...
        if any(
            keyword in user_input_lower
            for keyword in ["linkedin", "instagram", "twitter", "platform"]
        ):
            dialog.temporary_data["platform"] = self._detect_platform(user_input)
            return self._social_media_coach_ask_action(dialog)

        if any(
            keyword in user_input_lower for keyword in ["audit", "analiz", "profil"]
        ):
            return self._social_media_coach_profile_audit(dialog)

        if any(
            keyword in user_input_lower
            for keyword in ["strateji", "content", "içerik", "weekly"]
        ):
            return self._social_media_coach_content_strategy(dialog)

        if any(
            keyword in user_input_lower for keyword in ["marka", "brand", "kişisel"]
        ):
    ...
    def _social_media_coach_profile_audit(
        self, dialog: AdvisorDialog
    ) -> Dict[str, Any]:
        """Profil denetimi yap."""
        platform = dialog.temporary_data.get("platform", "LinkedIn")
        return {
    ...
    def _social_media_coach_content_strategy(
        self, dialog: AdvisorDialog
    ) -> Dict[str, Any]:
    ...
    def _social_media_coach_brand_assessment(
        self, dialog: AdvisorDialog
    ) -> Dict[str, Any]:
    ...
    def _handle_personal_assistant_input(
        self, dialog: AdvisorDialog, user_input: str
    ) -> Dict[str, Any]:
        ...
        if any(
            keyword in user_input_lower
            for keyword in ["özet", "summary", "bugün", "today"]
        ):
            return self._personal_assistant_daily_briefing()

        if any(
            keyword in user_input_lower
            for keyword in ["acil", "urgent", "görev", "task", "todo"]
        ):
            return self._personal_assistant_priority_tasks()

        if any(
            keyword in user_input_lower
            for keyword in ["hedef", "goal", "target", "objective"]
        ):
            return self._personal_assistant_goal_tracking()

        if any(
            keyword in user_input_lower
            for keyword in ["fırsat", "opportunity", "fikir", "idea"]
        ):
            return self._personal_assistant_opportunities()

        if any(
            keyword in user_input_lower
            for keyword in ["akşam", "evening", "gün sonu", "eod"]
        ):

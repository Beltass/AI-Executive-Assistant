...
SYSTEM_PROMPT = (
    ...
    "hemen yanında yaz. Emin değilsen rakam VERME; onun yerine yönü ve "
    "mekanizmayı anlat. Vaka sonucunu, şirket adını, tarihi veya yüzdeyi ASLA "
    "uydurma. Doğrulanması gereken her ifadeyi '(doğrulayın)' notuyla işaretle. "
    "'Bilinmiyor' kabul edilebilir bir cevaptır.\n\n"
    "MUTLAK DOĞRULUK KURALI (mevzuat): Mevzuat sık değişir. Madde numarası, "
    "tebliğ tarihi, ceza tutarı, süre, eşik gibi SPESİFİK ayrıntıları "
    "UYDURMA. Çerçevenin ve kurumun ADINI doğru ver (bunlar kalıcıdır), ama "
    "sayısal ve tarihsel ayrıntıya girme; ilkeyi, yapıyı, yönü ve pratikte ne "
    "anlama geldiğini anlat. Ayrıntı gerektiğinde 'uyum/hukuk biriminizle ve "
    "resmi kaynaktan doğrulayın' diye açıkça işaretle.\n\n"
...
class MarketIntelligenceAdvisor(Advisor):
    ...
    def _user_prompt(self) -> str:
        ...
        prompt = (
            ...
            "Sektör ve rekabet istihbaratı: (a) sektördeki teknoloji ve yapay "
            "zeka gelişmeleri — hangi kullanım senaryosu, hangi olgunluk "
            "seviyesinde, hangi ölçülebilir faydayla; (b) rakip ortamı — öne "
            "çıkan oyuncular, tedarikçi/çözüm ortağı ilişkileri, büyüme ve "
            "başarı temaları; (c) operasyonel etki — ekip yapısına, süreçlere "
            "ve KPI'lara (AHT, FCR, NPS, kayıp çağrı oranı) olası yansımalar; "
            "(d) 'ne yapmalı' — bu hafta gündeme alınabilecek 2-3 stratejik "
            "hamle ve her birinin riski.\n\n"
            ...
            "Kanıta dayalı müşteri deneyimi araştırması. Bu bölümün en derin "
            "kısmı yukarıdaki CX ODAĞI olsun. Şu blokları ver:\n"
            "(a) *🔍 Bugünün bulgusu*: odağa dair en önemli tespit; neyin "
            "bilindiğini, neyin tartışmalı olduğunu ve neyin bilinmediğini "
            "ayır.\n"
            "(b) *📊 Kanıt*: bulguyu destekleyen araştırma veya sektör raporu — "
            "kuruluş/rapor adını yaz. Rakam veriyorsan kaynağını yanına ekle; "
            "kaynağını söyleyemiyorsan rakam verme, mekanizmayı anlat.\n"
            "(c) *🏆 Başarı hikâyesi / vaka*: NE yapıldı, HANGİ metrik "
            "(NPS/CSAT/CES/FCR/churn/AHT) NE KADAR değişti, HANGİ sürede ve "
            "hangi koşulda. Rakamdan emin değilsen sayı uydurma; uygulamayı ve "
            ...
            "(e) *💛 Tutundurma bağlantısı*: bulgunun müşteri tutundurma "
            ...
            "güvenliği ve uyum. Bu bölümün en derin kısmı yukarıdaki DIŞ "
            "KAYNAK/UYUM ODAĞI olsun; diğer bloklar daha kısa kalabilir ama "
            "atlanmasın:\n"
            "(a) *📦 Dış kaynak yönetişimi*: due diligence ve tedarikçi ön "
            "eleme, yazılı risk analizi ve fayda-maliyet değerlendirmesi, "
            "yönetim kurulu / denetim komitesi onay akışı, sözleşme maddeleri "
            "(right-to-audit, alt yüklenici onayı, veri sahipliği ve iadesi, "
            "SLA ve ceza-teşvik, gizlilik, sorumluluk sınırı, çıkış planı), "
            "RFP/şartname kurgusu, fiyatlama modelleri (FTE bazlı, dakika/çağrı "
            "başı, hibrit, sonuç bazlı), geçiş (transition) planı ve SLA/KPI "
            "tasarımı — hangi KPI'yı tek başına ödüllendirmek hangi yan etkiyi "
            "doğurur. Sözleşmeye yazılacak madde önerisi ve tedarikçiye "
            "sorulacak somut sorular ver.\n"
            "(b) *🔐 Bilgi güvenliği kontrolleri*: çağrı merkezine ÖZGÜ kontrol "
            "seti — erişim (en az yetki, rol bazlı yetkilendirme, MFA, PAM, "
            "yetki gözden geçirme); kayıt ve saklama (çağrı ve ekran kaydı, "
            "şifreleme, saklama süresi ve imha, kart verisi için PCI-DSS "
            "beklentisi, pause-and-resume kaydın zayıflıkları, DTMF maskeleme "
            "ile kapsam daraltma); uç nokta (thin client/VDI, USB ve pano "
            "kısıtı, ekranda PII/PAN maskeleme, agent masaüstü DLP, BYOD "
            "yasağı); ağ ve entegrasyon (segmentasyon, güvenli API, veri "
            "minimizasyonu); personel (adli sicil/referans kontrolü, NDA, "
            "farkındalık eğitimi, içeriden tehdit); fiziksel (erişim kontrolü, "
            "temiz masa/temiz ekran, telefon-kamera yasağı); izleme (log "
            "saklama ve SIEM, sızma testi periyodu, zafiyet yönetimi). Her "
            "başlıkta 'nasıl doğrularsın — hangi kanıtı istersin' cevabını ver.\n"
            "(c) *⚖️ Uyum*: KVKK yükümlülükleri (veri sorumlusu/veri işleyen "
            "ayrımı ve veri işleyen sözleşmesinde bulunması gerekenler, "
            "aydınlatma ve açık rıza, saklama-imha politikası, VERBİS, veri "
            "ihlali bildirimi ve tedarikçiden beklenen bildirim SLA'i, yurt "
            "dışına aktarımda uygun güvence yöntemleri), ISO/IEC 27001 ve "
            "27002 beklentileri (sertifika kapsamı gerçekten hizmeti kapsıyor "
            "mu), SOC 2 Type 1/Type 2 farkı, PCI-DSS kapsamı, denetim izleri ve "
            "kanıt saklama. BDDK'nın destek hizmeti ve bilgi sistemleri "
            "düzenlemelerinin MANTIĞINI anlat; madde numarası ve tarih verme.\n"
            "(d) *🚨 Riskler ve erken uyarı sinyalleri*: yoğunlaşma "
            "(concentration) riski, dördüncü taraf alt yüklenici zinciri, "
            "uzaktan/evden çalışan temsilci riskleri, veri sızıntısı "
            "vektörleri (ekran fotoğrafı, kopyala-yapıştır, e-posta, USB, aşırı "
            "geniş rapor yetkisi), kalite-güvenlik ödünleşimi, gizli maliyetler "
            "ve vendor lock-in. Her risk için 'erken uyarı sinyali nedir' ve "
            "'sözleşmeye/operasyona ne konur' cevabını ver.\n"
            "(e) *🔬 Teknoloji ve yeni riskler*: konuşma/metin analitiği, agent "
            "assist, sesli ve yazılı botlar, CCaaS bulut geçişi, WFM ve kalite "
            "otomasyonu; ÖZELLİKLE üretken yapay zekânın çağrı merkezinde "
            "kullanımı ve veri koruma etkileri (transkript ve çağrı kaydının "
            "modele gitmesi, prompt üzerinden veri sızması, müşteri verisiyle "
            "model eğitimi, tedarikçinin YZ araç envanteri, çıktı denetimi, EU "
            "AI Act farkındalığı). Her biri için olgunluk seviyesi, ölçülebilir "
            "fayda ve bankaya özgü uyum/veri riski.\n"
            f"'📚 Kaynaklar' bölümünde her zaman EN AZ bir resmî düzenleyici "
            f"veya standart kuruluşu kaynağı bulunsun ({OFFICIAL_SOURCES}) ve "
            "derin URL yerine kök alan adını ver.\n\n"
            ...
            "ortak kurallardaki 300-500 kelime hedefi BÜTÜN için 800-1000 "

"""AI mastery coach — Yapay Zeka Ustalığı Koçu.

A teacher, not a news desk. Where :mod:`ai_assistant.advisors.ai_news` reports
what happened in AI, this persona builds the user's own SKILL with AI tools —
deliberately, from zero to advanced — one lesson a day.

How the progression works:

* **Level** (``AI_MASTERY_LEVEL``: ``temel`` → ``orta`` → ``ileri``) sets the
  depth: the same topic is taught very differently to a beginner and to someone
  already shipping automations.
* **Curriculum rotation** picks the day's topic from a fixed syllabus
  (prompt engineering, system prompts and context, chain-of-thought and
  structured output, RAG, agents and tool use, automation with n8n/Zapier/Make,
  API usage, evaluating outputs, avoiding hallucinations, data privacy, cost
  control, choosing the right model). Rotating by day-of-year means consecutive
  days never repeat and the whole syllabus is covered over time.

Every section must deliver three things the user asked for by name:
**hap bilgi** (one crisp, immediately usable nugget), **eğitim videoları** and
**ücretsiz sertifikalı eğitimler** — plus a hands-on ``✅ Bugünün görevi`` that
is actually run inside an AI tool.

Grounding, because a fabricated course link is worse than no link: an RSS feed
of real training/certificate announcements is fetched OUTSIDE the LLM call and
handed to the persona, and the persona may otherwise reference only well-known,
stable providers by ROOT DOMAIN (Google Cloud Skills Boost, Microsoft Learn,
DeepLearning.AI, Coursera, edX, Hugging Face, Kaggle Learn, freeCodeCamp,
Anthropic/OpenAI docs) — never an invented deep URL — always with the
"🔎 Bağlantıları açılışta doğrulayın." caveat.

Feed items are filtered through the :mod:`ai_assistant.memory` ledger first, so
the extra daily runs discuss only announcements the user has not seen yet.

Configuration (via environment):
    AI_MASTERY_LEVEL     temel | orta | ileri (default ``temel``).
    AI_MASTERY_RSS_URL   Feed of free AI training/certificate news. Defaults to
                         a Turkish Google News search so real links exist out of
                         the box.
...
SYSTEM_PROMPT = (
    "Sen 'Yapay Zeka Ustalığı Koçu'sun: yapay zekâ uygulamalarını sıfırdan "
    "ileri seviyeye kadar öğreten, sabırlı ve çok pratik bir eğitmensin. "
    "Türkçe konuşuyorsun. Öğrencin teknik bir uzman değil; onu her gün bir "
    "adım ileri taşımak, kullandığı araçlarda gerçekten ustalaştırmak "
    "istiyorsun. Anlatımın somut: her kavramı örnekle, kopyalanabilir istem "
    ...
    "ana adresini verip arama terimi önerirsin."
)
...
class AiMasteryAdvisor(Advisor):
    key = "ai_mastery"
    title = "Yapay Zeka Ustalığı Koçu"

    # Its free-training feed is a genuine source of NEW findings between runs.
    ...
    def _fetch_items(self) -> List[FeedItem]:
        ...
        try:
            ...
            return self._items
        # Only items the user has not already been told about.
    ...
    def _user_prompt(self) -> str:
        ...
        prompt = (
            ...
            "Bugünün dersini bu konu etrafında kur; konuyu seviyeye göre "
            "derinleştir ve bir sonraki adımı da göster (bu konuda "
            "ustalaşınca sırada ne var).\n\n"
            "Bölümü şu YAPIYLA ver:\n"
            "1) *💊 Hap bilgi*: bugünün tek cümlelik, hemen uygulanabilir "
            "püf noktası — kalın yazılmış, akılda kalıcı.\n"
            "2) *📖 Dersin özü*: konuyu adım adım öğret; en az bir "
            "KOPYALANABİLİR istem (prompt) kalıbı ver ve aynı isteğin zayıf ve "
            "güçlü hâlini yan yana göster.\n"
            "3) *🎬 Eğitim videoları*: 2-3 video kaynağı öner. Kanal/sağlayıcı "
            "adını ve ne arayacağını yaz; derin video URL'i UYDURMA, kanalın "
            "veya platformun ana adresini ver.\n"
            "4) *🎓 Ücretsiz sertifikalı eğitimler*: 2-3 ücretsiz (veya "
            ...
            "5) *⚠️ Sık yapılan hata*: bu konuda yeni başlayanların düştüğü bir "
            "tuzak ve düzeltmesi.\n"
            "6) *✅ Bugünün görevi*: bugün 15-30 dakikada bir yapay zekâ "
            "aracında BİZZAT çalıştırılacak somut bir alıştırma — ne "
            "yazılacağını (istemi aynen ver), neye bakılacağını ve başarının "
            "nasıl anlaşılacağını yaz.\n\n"

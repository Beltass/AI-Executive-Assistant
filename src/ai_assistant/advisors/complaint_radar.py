...
SYSTEM_PROMPT = (
    "Sen bankacılık ve finans sektöründe müşteri şikâyet yönetimi, itibar "
    "takibi ve müşterinin sesi (VoC) programları kurmuş kıdemli bir "
    "danışmansın. Türkçe yazıyorsun. İşin şikâyet başlıklarını saymak değil, "
    "ARKASINDAKİ MEKANİZMAYI okumaktır: hangi süreç kırıldığında bu şikâyet "
    "doğar, kurum nasıl yanıt verdiğinde şikâyet kapanır, hangi yanıt biçimi "
    "şikâyeti büyütür.\n\n"
    "ÇERÇEVE BİLGİSİ (bunlar gerçek ve kalıcı isimlerdir): hizmet telafisi "
    "(service recovery) ve telafi paradoksu; algılanan adaletin üç boyutu "
    "(sonuç, süreç, etkileşim); ilk temasta çözüm (FCR) ve tekrar temas; "
    "müşteri eforu (CES); kapalı döngü (closed-loop) geri bildirim; şikâyet "
    "kök neden analizi. Türkiye tarafında şikâyet başvuru yolları olarak "
    "kurumun kendi müşteri iletişim merkezi, TBB Müşteri Şikayetleri Hakem "
    "Heyeti ve BDDK'nın şikâyet kanalı vardır.\n\n"
    "KANIT DİSİPLİNİ (mutlak kural): Şikâyet sayısı, oran veya sıralama "
    "SADECE elindeki akıştan çıkarılabiliyorsa verilir ve nereden geldiği "
    "yazılır. Kurum adı, ceza, tarih veya yüzde UYDURMA. Bir marka hakkında "
    "elinde başlık yoksa o marka için 'bu akışta veri yok' de; tahmin yürütme. "
    "'Bilinmiyor' kabul edilebilir bir cevaptır. Doğrulanması gereken her "
    "ifadeyi '(doğrulayın)' notuyla işaretle.\n\n"
    "Ton: soğukkanlı, kanıta bakan, suçlayıcı olmayan. Şikâyet eden müşteriyi "
    "de, adı geçen kurumu da karikatürleştirme."
)
...
class ComplaintRadarAdvisor(Advisor):
    ...
    def _user_prompt(self) -> str:
        ...
        return (
            "Bugün için bankacılık ve finans kurumları hakkındaki müşteri "
            "şikâyetlerini okuyan TEK bir itibar radarı brifingi yaz.\n"
            f"{context}\n"
            ...
            f"{themes}\n\n"
            "*🏛️ Kurumlar nasıl yanıt veriyor*: gözlenen çözüm yaklaşımı ve "
            "alınan önlemler — yanıt süresi, telafi biçimi, kanal değişikliği, "
            "süreç/ürün düzeltmesi. Hangi yanıt biçiminin şikâyeti kapattığını, "
            "hangisinin büyüttüğünü hizmet telafisi ve algılanan adalet "
            "çerçevesiyle açıkla.\n\n"
            "*📈 Hacim ve yön*: şikâyet hacminin zaman içindeki değişimi — "
            "artıyor mu, azalıyor mu, mevsimsel/kampanya kaynaklı bir sıçrama "
            "mı. Elindeki başlık sayısı bir eğilim iddiası için yetersizse "
            "bunu açıkça söyle ve neyin ölçülmesi gerektiğini yaz.\n\n"
            "*⚔️ Rakip kıyası*: aynı temada rakipler nerede duruyor; kim hangi "
            "konuda daha çok/az anılıyor ve bu fark neyi gösteriyor.\n\n"
            "*🧭 Yorum*: bu tablo bir çağrı merkezi operasyonu için ne anlama "
            "geliyor — hangi KPI'ya (FCR, tekrar temas, AHT, CES, NPS) basınç "
            "yapar, hangi kök neden en pahalıdır, hangi erken uyarı sinyali "
            "izlenmeli. Bu bölüm LİSTE DEĞİL, YORUM olsun.\n\n"
            "'✅ Bugünün görevi' bölümünde bugün 15-30 dakikada yapılabilecek "
            "TEK somut eylem yaz — tercihen doğrulanabilir bir kontrol (örn. "
            "'geçen hafta kapatılan şikâyetlerden 10 tanesini aç, kaçında "
            "aynı müşteri 7 gün içinde tekrar aramış bak').\n\n"
            + RICH_BRIEFING_GUIDE
            + self._feed_block()
        )
